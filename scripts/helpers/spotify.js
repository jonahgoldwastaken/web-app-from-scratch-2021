...
async function fetchSpotifyPlaylist(id) {
  const token = getToken()
  const playlist = fetchAndParseJSON(
  ...
  return await fetchAndParseJSON(
    `https://api.spotify.com/v1/users/${id}/playlists`,
    {
      ...
      body: JSON.stringify({
        ...
        description: 'Generated by Triptop @ triptop.app',
      }),
    }
  )
}
...
/**
 * Gets the profile from sessionStorage
 * @returns {object} Spotify UserObject
 */
function getProfile() {
  try {
    const profile = JSON.parse(sessionStorage.getItem('spotify-profile'))
    return profile
  } catch {
    return null
  }
...
/**
 * Saves the profile in sessionStorage
 */
...
/**
 * Checks if the Spotify Token is present
 * @returns {boolean}
 */
